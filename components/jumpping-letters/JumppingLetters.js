...
const JumppingLettersFunc = () => {
	...
	const string = '"Be water, my friend"'
		...
		<div>
				...
				<div className="inner">
					{
						lettersArray.map((letter, idx) => {
							// console.log(letter.length, letter + idx)
							return (
								letter === ' '
									?
									<div key={idx} className="letter" data-title="&nbsp;">&nbsp;</div>
									:
									idx === 9
										?
										<br />
										:
										<div key={idx} className="letter" data-title={letter}>{letter}</div>
							)
						})
					}
			...
			<style jsx>{`
				...
				.letter {
					...
					text-shadow: -15px 10px 10px rgba(0, 0, 0, 0.3), 0 20px 50px rgba(0, 0, 0, 0.3);
...
export default JumppingLettersFunc